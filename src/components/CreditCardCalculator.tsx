...
const CreditCardCalculator: React.FC<CreditCardCalculatorProps> = ({
  config,
}) => {
  ...
  const paidOffBalance =
    inputs.monthlySpend * (inputs.balanceCarriedPercent / 100);
  ...
  const calculateInvestmentFinalValue = (
    years: number,
    rate: number,
    monthly: number,
  ) => {
  ...
  const investmentFinalValue = calculateInvestmentFinalValue(
    investmentYears,
    investmentRate,
    monthlySavings,
  );
            ...
            <div className="relative">
              ...
              <input
                ...
                onChange={(e) => {
                  const value =
                    e.target.value === "" ? 0 : Number(e.target.value);
                  const newBalanceCarriedPercent =
                    inputs.monthlySpend > 0
                      ? Math.min(100, (value / inputs.monthlySpend) * 100)
                      : 0;
                  handleInputChange(
                    "balanceCarriedPercent",
                    newBalanceCarriedPercent,
                  );
                }}
                onBlur={(e) => {
                  if (e.target.value === "" || Number(e.target.value) < 0) {
      ...
      <div className="relative z-10 max-w-7xl mx-auto py-8 px-4">
        ...
        <div className="text-center mb-12">
          <div className="flex justify-between items-center mb-8">
            ...
            {config.mode !== "auto" && <div className="w-16"></div>}{" "}
            {/* Spacer when toggle hidden */}
          </div>
        </div>
              ...
              <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 mb-6 lg:mb-8">
                          ...
                          onChange={(e) => {
                            ...
                            if (inputs.monthlySpend > 0) {
                              const percentage = Math.round(
                                (dollarAmount / inputs.monthlySpend) * 100,
                              );
                              handleInputChange(
                                "balanceCarriedPercent",
                ...
                <div className="lg:w-2/3">
                  <div className="space-y-4">
                    ...
                    >
                      Let's imagine you put that saved money into an investment
                      portfolio for the next ten years
                    </h2>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-6 lg:mt-8">
                ...
                >
                  <p className="text-sm sm:text-base md:text-lg mb-2">
                    By switching to <strong>SPZero's 0% APR card</strong>, you
                    could save{" "}
                    <strong style={{ color: "#0067f7" }}>
                      ${totalInterestSaved.toLocaleString()}
                    </strong>{" "}
                    in interest payments over the next {investmentYears} years,
                    which if invested, could reach a value of{" "}
                    <strong style={{ color: "#0067f7" }}>
                      ${investmentFinalValue.toLocaleString()}
                    </strong>
                  </p>
                </div>
                ...
                {/* Disclaimer */}
                <div
                  className={`flex items-start ${darkMode ? "text-white" : "text-gray-600"}`}
                >
                  ...
                  <div>
                    ...
                    <p className="text-xs leading-relaxed">
                      ...
                      Assumes monthly contributions with {investmentRate}%
                      annual return, compounded monthly. Actual investment
                      returns may vary significantly. Past performance does not
                      guarantee future results. Consider consulting with a
                      financial advisor before making investment decisions. This
                      tool is not financial advice.
                    </p>
                  </div>
                </div>
              </div>
        ...
        <div
          className={`relative overflow-hidden rounded-lg shadow-lg mt-8`}
          style={{
            background: darkMode
              ? "linear-gradient(to bottom right, #8b3dff, #000)"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              ...
              <div
                className={`p-4 sm:p-6 rounded-lg ${
                  darkMode
                    ? "bg-gray-800/50 border border-purple-700/30"
              ...
              >
                ...
                >
                  $
                  {(annualInterest * 5).toLocaleString(undefined, {
                    minimumFractionDigits: 2,
                    maximumFractionDigits: 2,
                  })}
                </div>
